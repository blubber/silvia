...
Cw = 0.23 * 4176.8 # 330 mL of water
...
Cm =  ((math.pi * r**2  - math.pi * (r - d/2)**2) * h + 2 * (math.pi * r**2 * d)) * rhoBrass * cpBrass
...
def model (fn, y, t):
    ...
    '''
    return [((-1/(Rf*Cw))*y[0] + (1/(Cw* Rf))*y[1] + fn(t, y[1])/Cw), \
        ((1/(Rf*Cm))*y[0] -(1/(Rf*Cm) + 1/(Re*Cm))*y[1] +(1/(Re*Cm)) * Te )]
